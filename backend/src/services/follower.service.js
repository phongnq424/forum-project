...
const FollowerService = {
    ...
    getFollowers: async (userId, query) => {
        ...
        if (query.search) where.follower = { username: { contains: query.search, mode: 'insensitive' } }

        const followers = await prisma.follower.findMany({
            where,
            skip,
            take,
            include: { follower: { select: { id: true, username: true, Profile: { select: { avatar: true } } } } }
        })
        const total = await prisma.follower.count({ where })
        return { data: followers, pagination: { total, page: parseInt(query.page) || 1, limit: take, totalPages: Math.ceil(total / take) } }
    },
    ...
    getFollowing: async (userId, query) => {
        ...
        if (query.search) where.followed = { username: { contains: query.search, mode: 'insensitive' } }

        const following = await prisma.follower.findMany({
            where,
            skip,
            take,
            include: { followed: { select: { id: true, username: true, Profile: { select: { avatar: true } } } } }
        })
        const total = await prisma.follower.count({ where })
        return { data: following, pagination: { total, page: parseInt(query.page) || 1, limit: take, totalPages: Math.ceil(total / take) } }
    }
}
