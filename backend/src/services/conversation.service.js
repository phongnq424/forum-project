...
const ConversationService = {
    ...
    getMessagesByConversation: async (conversationId, userId, { viewerId, blockContext } = {}) => {
        ...
        if (conv.type === 'CHAT') {
            const otherUserId = conv.ConversationUser
                .find(u => u.user_id !== userId)?.user_id
    ...
    sendMessage: async (conversationId, senderId, content, { viewerId, blockContext }) => {
        const conv = await prisma.conversation.findUnique({
            where: { id: conversationId },
            include: { ConversationUser: true }
        })
        ...
        const otherUserId = conv.ConversationUser
            .find(u => u.user_id !== senderId)?.user_id

        if (
            conv.type === 'CHAT' &&
            blockContext?.blockedSet?.has(otherUserId)
        ) {
            throw new Error('User is blocked')
        }
    ...
    sendDirectMessage: async (fromUserId, toUserId, content, { blockContext } = {}) => {
        // 1. block check
